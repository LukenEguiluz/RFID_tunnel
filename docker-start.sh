...
HP="${GATEWAY_HTTP_PORT:-38080}"

echo ""
echo "✅ Gateway iniciado y expuesto en el contenedor."
echo ""
echo "📍 URLs en esta máquina (puerto host ${HP}; contenedor interno 8080):"
echo "   - API REST:  http://localhost:${HP}/api"
echo "   - Estado:    http://localhost:${HP}/api/status"
echo "   - Health:    http://localhost:${HP}/api/health"
echo "   - WebSocket: ws://localhost:${HP}/ws/events"
echo ""
echo "📍 Para la webapp (misma red ZeroTier), usa la IP de esta VM:"
echo "   - API:       http://${ZT_IP}:${HP}"
echo "   - WebSocket: ws://${ZT_IP}:${HP}/ws/events"
echo ""
